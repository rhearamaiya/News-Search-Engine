...
public class App {
    public static String getObject(String bucketName, S3Client s3){
        ...
        if(System.getenv("COMMON_CRAWL_FILENAME") == null || System.getenv("COMMON_CRAWL_FILENAME").equals("")) //if no filename is given then just give the last one
        {
            ...
            ListObjectsRequest listObjects = ListObjectsRequest
                    .builder()
                    .bucket(bucketName)
                    .prefix("crawl-data/CC-NEWS/2021/02")
                    .build();
    ...
    public static void main(String[] args) throws IOException {
        ...
        //https://www.programcreek.com/java-api-examples/?api=org.archive.io.ArchiveReader
        FileInputStream is = new FileInputStream(f);
        reader = WARCReaderFactory.get(key1, is, true);

        ElasticSearch es = new ElasticSearch("es");
        es.NewElasticSearchIndex(); //create new elastic search index
        ...
        for (org.archive.io.ArchiveRecord r : reader) {
            ...
            if(r.getHeader().getHeaderValue("Content-Type").equals("application/http; msgtype=response")) //from PM office hours
            {
                ...
                while(offset != -1){
                    ...
                    String content = new String(rawData, StandardCharsets.UTF_8.name()); //from campuswire, https://github.com/Smerity/cc-warc-examples/blob/master/src/org/commoncrawl/examples/WARCReaderTest.java
                    ...
                    try {
                        ...
                        String body_text = doc.body().text(); //extract plain text source: https://jsoup.org/cookbook/extracting-data/attributes-text-html
